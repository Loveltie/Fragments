...
async function loadWords() {
  ...
  // Show dedup button only when duplicates exist
  const seen = new Set();
  const dupeCount = words.reduce((n, w) => {
    const key = w.word.toLowerCase().trim();
    if (seen.has(key)) return n + 1;
    seen.add(key);
    return n;
  }, 0);
  dedupBtn.hidden      = dupeCount === 0;
  dedupBtn.textContent = `Remove ${dupeCount} duplicate${dupeCount === 1 ? '' : 's'}`;
  dedupBtn.onclick = async () => {
    dedupBtn.disabled = true;
    const result = await sendMsg({ action: 'deduplicate' });

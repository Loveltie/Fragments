...
// Remove duplicate cards (same word, case-insensitive).
// Keeps the entry with the most filled-in fields; deletes the rest.
// Returns { removed: N }.
async function deduplicateWords() {
  const all = await getAllWords();

  // Group by normalised word
  const groups = {};
  for (const w of all) {
  ...
  // Score each entry: translation=4, example=2, article=1, reviewed=1
  const score = w =>
    (w.translation ? 4 : 0) +
    (w.example     ? 2 : 0) +
    (w.article     ? 1 : 0) +
  ...
  for (const entries of Object.values(groups)) {
    ...
    // Sort: highest score first, then newest first as tiebreaker
    entries.sort((a, b) =>
      score(b) - score(a) ||
      new Date(b.createdAt) - new Date(a.createdAt)
    );
    for (let i = 1; i < entries.length; i++) toDelete.push(entries[i].id);
  }
